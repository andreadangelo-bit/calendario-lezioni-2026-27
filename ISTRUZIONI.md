...
Per **ogni** combinazione possibile di Anno di corso, Indirizzo e Lingue — esattamente come la sceglierebbe uno studente nel calendario — la pagina mostra quante lezioni cadono in ciascun giorno della settimana e il totale. È utile per individuare a colpo d'occhio giornate molto cariche o vuote per un dato profilo, senza dover aprire il calendario di ogni singola combinazione una per una.

- L'elenco è generato **automaticamente** dai dati presenti nel foglio Google (stessa fonte di `index.html`): non serve inserire o mantenere nulla a mano.
- Tre menu a tendina in alto (Anno, Indirizzo, Lingua) permettono di **filtrare** la tabella per trovare più in fretta le righe che interessano; non limitano l'elenco generato, che resta sempre completo — sono solo un aiuto per la ricerca. Il pulsante "Reimposta filtri" li azzera tutti.
- Si può ordinare la tabella cliccando sull'intestazione di qualsiasi colonna (es. cliccare "Tot." per vedere subito le combinazioni con più lezioni in totale, o su un giorno per vedere quelle più cariche in quel giorno).
- Cliccando su **"Apri calendario →"** in una riga si apre, in una nuova scheda, lo stesso calendario (`index.html`) che vedrebbe uno studente con quel profilo già selezionato — utile per controllare nel dettaglio orari, aule e docenti di quella combinazione specifica.
