...
**Sfoglia tutte le combinazioni** — l'elenco è generato automaticamente dai dati presenti nel foglio Google (stessa fonte di `index.html`): non serve inserire o mantenere nulla a mano. Tre menu a tendina (Anno, Indirizzo, Lingua) permettono di filtrare la tabella per trovare più in fretta le righe che interessano; non limitano l'elenco generato, che resta sempre completo — sono solo un aiuto per la ricerca. Si può ordinare la tabella cliccando sull'intestazione di qualsiasi colonna (es. "Ore tot." per vedere subito le combinazioni più cariche).
